...
public protocol CheckJailBreak: AnyObject {
...
public extension CheckJailBreak where Self: AnyObject {
    ...
    func assignJailBreakCheckType(type: TypeOfJailBreakCheckAPI) {
        ...
        if !isSimulator {
            ...
            guard UIApplication.shared.canOpenURL(URL(string: "cydia://")!) else {
                ...
                self.sendTheStatusOfJailBreak(isJailBreak: checkStatus)
                return
            }
            self.sendTheStatusOfJailBreak(isJailBreak: true)
        }
        self.sendTheStatusOfJailBreak(isJailBreak: false)
    }
    ...
    func isJailBrokenFilesPresentInTheDirectory() -> Bool {
        ...
        filesPathToCheck.forEach {
            checkFileIfExist =  fm.fileExists(atPath: $0)
            if checkFileIfExist {
                return
            }
        }
