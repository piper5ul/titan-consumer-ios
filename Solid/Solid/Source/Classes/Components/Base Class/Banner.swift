...
open class Banner: UIView {
    ...
    @objc open var minimumHeight: CGFloat = 65
    ...
    @objc open var dismissesOnSwipe = false
    ...
    private func initializeSubviews() {
        ...
        if image == nil {
            ...
            contentView.addConstraint(imageView.constraintWithAttribute(.centerY, .equal, to: titleLabel))
		...
		let labelConstraintFormat = "-(5)-[labelView]-(8)-|"
		let hcontraintFormat = "H:"
        let constraintFormat = "\(hcontraintFormat)\(leftConstraintText)\(labelConstraintFormat)"
        ...
        labelView.addConstraints(NSLayoutConstraint.defaultConstraintsWithVisualFormat("V:|-(0)-[titleLabel]-(0)-|", views: views))
    }
    ...
    private func adjustHeightOffset() {
        ...
        if superview === Banner.topWindow() && self.position == .top {
            ...
            contentTopOffsetConstraint.constant = 30
            contentBottomOffsetConstraint.constant = 0
            minimumHeightConstraint.constant = statusBarHeight > 0 ? minimumHeight : 20
        } else {
    ...
    open func show(_ view: UIView? = nil, duration: TimeInterval? = nil) {
        ...
        if adjustsStatusBarStyle {
            let window = UIApplication.shared.windows.filter {$0.isKeyWindow}.first
            oldStatusBarStyle = window?.windowScene?.statusBarManager?.statusBarStyle
        } else {
